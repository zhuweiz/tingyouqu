...
Page({
  ...
  this_value: function(e) {
    ...
    if (formData) { // 搜索
      wx.redirectTo({
        url: '../train_search/train_search?formData=' + formData + '&name=' + formData + '&id=' + this.data.id
      })
      ...
      wx.setStorageSync('youwan', searchData);
    } else {
  ...
  openLocationsercher: function() { //获取搜索缓存
    console.log(wx.getStorageSync('youwan'))
    this.setData({
      sercherStorage2: wx.getStorageSync('youwan') || [],
      StorageFlag: true,
      listFlag: true,
    })
  },
  clearSearchStorage: function() { //清除搜索历史
    wx.removeStorageSync('youwan')
    this.setData({
      sercherStorage2: [],
      StorageFlag: false,
    })
